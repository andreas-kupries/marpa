...
marpatcl_rtc_lexer_init (marpatcl_rtc_p p)
{
    ...
    marpatcl_rtc_dynset_cons (ACCEPT, SPEC->lexemes + ALWAYS.size);
...
{
    marpa_g_unref (LEX.g);
    marpatcl_rtc_dynset_release (ACCEPT);
...
marpatcl_rtc_lexer_enter (marpatcl_rtc_p p, int ch)
{
    ...
    marpatcl_rtc_stack_push (LX.lexeme, ch);
...
marpatcl_rtc_lexer_acceptable (marpatcl_rtc_p p)
{
    ...
    buf = marpatcl_rtc_dynset_dense (ACCEPT);
    n   = marpa_r_terminals_expected (PARS_R, buf);
    // TODO: down-convert parser symbols to lexer (lexeme ACS)
    marpatcl_rtc_dynset_link    (ACCEPT, n);
    marpatcl_rtc_dynset_include (ACCEPT, ALWAYS.size, ALWAYS.data);

    // And feed it into the new lexer recce
    n = marpatcl_rtc_dynset_size (ACCEPT);
    if (n) {
	int k;
	buf = marpatcl_rtc_dynset_dense (ACCEPT);
	for (k=0; k < n; k++) {
